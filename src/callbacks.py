...
def fig2a(plt, cdf):
    ...
    plt.vlines(50, -.02, 1.02, color='r', linestyle='--', 
...
def fig12(plt, shs):    
    ...
    cen = np.where(shs.index.isin([0, 4, 7, 9, 20, 24, 25, 28, 30, 31, 36, 38]))[0]
    cen_sh = shs.iloc[cen].sh.values
    shadows = np.where(shs.index.isin([19, 33,  2,  18, 15, 16,  1,  5, 32, 11]))[0]
    shadows_sh = shs.iloc[shadows].sh.values
    adbworm = np.where(shs.index.isin([21]))[0]
    adbworm_sh = shs.iloc[adbworm].sh.values
    plt.xticks(list(cen)+list(shadows)+list(adbworm), [])
    plt.scatter(cen, cen_sh, s=30, color='g', marker='o', label='Cyberattacks')
    plt.scatter(shadows, shadows_sh, s=30, color='b', 
                marker='s', label='Survey scans')
    plt.scatter(adbworm, adbworm_sh, s=30, color='r', 
                marker='*', label='Sporadically-focused traffic')
    plt.legend()
    plt.xlim(-1)
    plt.ylim(None,1.05)
