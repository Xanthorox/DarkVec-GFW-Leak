...
GROUNDTRUTH = f'{DATA}/groundtruth/ground_truth_full.csv'
MODELS = f'{DATA}/models_vec'
...
for i in range(2, 32):
    if i<10:
        traces_path+='file://'+TRACES[:-12]+f'trace-{START}0{i}*/packets.log,'
    else:
        traces_path+='file://'+TRACES[:-12]+f'trace-{START}{i}*/packets.log,'

DEBUG = traces_path[:-1]

for i in range(2, 32):
    if i<10:
        traces_path+='file://'+root_path+TRACES[:-12]+f'trace-{START}0{i}*/packets.log,'
    else:
        traces_path+='file://'+root_path+TRACES[:-12]+f'trace-{START}{i}*/packets.log,'
